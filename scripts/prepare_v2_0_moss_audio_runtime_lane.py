#!/usr/bin/env python3
"""Record aggregate-only MOSS-Audio-4B runtime/cache lane readiness."""
...
RUN_ID = "v2_0_multimodal_batch1_moss_audio_4b_runtime_lane_2026_06_01"
...
CACHE_DIR = "models--OpenMOSS-Team--MOSS-Audio-4B-Instruct"
...
def write_readme(out_dir: Path, summary: dict[str, Any]) -> None:
    text = f"""# MOSS-Audio-4B Runtime Lane Preparation

Date: 2026-06-01

Status: {summary['status']}

本紀錄只保存 MOSS-Audio-4B runtime lane aggregate status，不保存任何逐字稿、
...
audio_loader_policy={summary['audio_loader_policy']}
...
def main() -> int:
    ...
    if not cache["cache_present"] or not cache["expected_snapshot_present"]:
        missing.append("missing_moss_audio_4b_model_cache")
    if not OFFICIAL_REPO_DIR.exists():
        missing.append("missing_openmoss_moss_audio_repo")

    status = "ready_for_moss_audio_4b_one_row_smoke" if not missing else "blocked_before_moss_audio_4b_one_row_smoke"
    summary = {
        "run_id": RUN_ID,
        "generated_at_unix": int(time.time()),
        "gate": "MOSS-Audio-4B isolated runtime/cache lane preparation",
        "status": status,
        "model_id": MODEL_ID,
        ...
        "next_gate": "run_moss_audio_4b_one_row_transcript_only_smoke" if status == "ready_for_moss_audio_4b_one_row_smoke" else "fix_moss_audio_4b_runtime_lane_then_rerun_preflight",
        "privacy": {
    ...
    (args.out_dir / "moss_audio_4b_runtime_lane_summary.json").write_text(
