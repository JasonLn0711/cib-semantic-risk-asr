#!/usr/bin/env python3
"""Validate aggregate-only MiniCPM-o 4.5 sentinel repair records."""
...
RUN_DIR = Path("70_experiments/runs/v2_0_multimodal_batch1_minicpm_o_4_5_sentinel_repair_2026_06_01")
REQUIRED_FILES = {
    "README.md",
...
def main() -> int:
    ...
    if summary.get("status") not in {
        "minicpm_o_4_5_sentinel_repair_complete",
        "minicpm_o_4_5_sentinel_repair_failed",
    }:
        raise SystemExit("unexpected_status")
    behavior = read_tsv(args.run_dir / "behavior_summary.tsv")
    if summary.get("status") == "minicpm_o_4_5_sentinel_repair_complete" and len(behavior) != 6:
        raise SystemExit("behavior_summary_must_have_6_rows_for_complete_run")
    read_tsv(args.run_dir / "runtime_environment_summary.tsv")
    print(f"minicpm_o_4_5_sentinel_repair_record_ok {args.run_dir}")
    return 0
