#!/usr/bin/env python3
"""Validate aggregate-only MOSS-Audio-4B one-row smoke record."""
...
RUN_DIR = Path("70_experiments/runs/v2_0_multimodal_batch1_moss_audio_4b_one_row_smoke_2026_06_01")
...
def main() -> int:
    ...
    if summary.get("smoke_status") != "completed":
        raise SystemExit("moss_audio_4b_smoke_not_completed")
    if summary.get("valid_text_outputs") != 1:
        raise SystemExit("moss_audio_4b_missing_valid_text_output")
    ...
    print(f"moss_audio_4b_one_row_smoke_record_ok {RUN_DIR}")
    return 0
