#!/usr/bin/env python3
"""Validate aggregate-only Qwen2.5-Omni one-row smoke records."""
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        nargs="?",
        type=Path,
        default=Path("70_experiments/runs/v2_0_multimodal_batch1_qwen_one_row_smoke_2026_05_31"),
    )
    ...
    if summary.get("manifest_field_names_tracked") is not False:
        raise SystemExit("manifest field names must not be tracked")
    print(f"qwen_one_row_smoke_record_ok {args.run_dir}")
    return 0
