#!/usr/bin/env python3
"""Validate aggregate-only MOSS-Audio-8B one-row smoke record."""
...
RUN_DIR = Path("70_experiments/runs/v2_0_multimodal_batch1_moss_audio_8b_one_row_smoke_2026_06_01")
...
def main() -> int:
    ...
    if summary.get("smoke_status") == "completed":
        if summary.get("valid_text_outputs") != 1:
            raise SystemExit("moss_audio_8b_missing_valid_text_output")
        ...
        if summary.get("promotion_decision") not in {"blocked_runtime_resource", "blocked_runtime_dependency"}:
            raise SystemExit("failed_moss_audio_8b_gate_must_be_blocked")
        if not str(summary.get("failure_mode", "")).startswith(("resource_error:", "runtime_error:")):
            raise SystemExit("failed_moss_audio_8b_gate_missing_classified_failure_mode")
    else:
    ...
    print(f"moss_audio_8b_one_row_smoke_record_ok {RUN_DIR}")
    return 0
