#!/usr/bin/env python3
"""Validate aggregate-only Step-Audio-2-mini sentinel controls."""
...
RUN_DIR = Path("70_experiments/runs/v2_0_multimodal_batch1_step_audio_sentinel_controls_2026_06_01")
REQUIRED_FILES = {
    "README.md",
...
def main() -> int:
    ...
    if summary.get("status") == "step_audio_sentinel_controls_complete" and len(behavior) != 6:
        raise SystemExit("behavior_summary_must_have_6_rows")
    if summary.get("promotion_decision") == "promote_to_15_row_candidate_pool":
        if int(summary.get("sentinel_pass_rows", 0)) != 6:
            raise SystemExit("cannot_promote_without_6_sentinel_pass_rows")
    print(f"step_audio_sentinel_controls_record_ok {args.run_dir}")
    return 0
