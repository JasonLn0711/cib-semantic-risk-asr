...
save_dual("fig3_predictor_auc", function() {
  par(mar = c(5, 5, 3, 1))
  y <- pred$point_auc
  bp <- barplot(y, names.arg = pred$label, col = bar_cols[1:4], ylim = c(0, 1), ylab = "AUC", main = "Pilot predictor comparison with row-clustered intervals")
  ...
  mtext("Pilot layer: cluster unit is audio row; 30 clusters; 90 model assessments", side = 1, line = 4, cex = 0.78)
}, width = 7, height = 5)
