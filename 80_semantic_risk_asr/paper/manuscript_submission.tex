...
\subsection{Table 1. Main ASR Benchmark
...
Rows
\end{minipage} & \begin{minipage}[b]{\linewidth}\RaggedLeft
\texttt{\seqsplit{cer\_zh\_micro}}
\end{minipage} & \begin{minipage}[b]{\linewidth}\RaggedLeft
\texttt{\seqsplit{wer\_zh\_jieba\_micro}}
\end{minipage} & \begin{minipage}[b]{\linewidth}\RaggedLeft
Unsafe downrouting
\end{minipage} & \begin{minipage}[b]{\linewidth}\RaggedLeft
High-risk missed
...
\end{minipage} \\
\midrule\noalign{}
\texttt{\seqsplit{breeze\_asr25\_partial\_encoder\_legacy\_best\_test\_split}} &
Breeze-ASR-25 partial encoder & 258 & 15.04 & 21.53 & 7 & 4 & 0 &
Scope-controlled split/model comparison \\
\texttt{\seqsplit{breeze\_asr25\_lora\_legacy\_best\_test\_split}} & Breeze-ASR-25
LoRA & 258 & 18.23 & 25.59 & 10 & 7 & 0 & Scope-controlled split/model
comparison \\
\texttt{\seqsplit{breeze\_asr25\_base\_test\_split}} & Breeze-ASR-25 base & 258 &
22.72 & 30.39 & 34 & 30 & 0 & Scope-controlled split/model comparison \\
\texttt{\seqsplit{breeze\_asr26\_test\_split}} & Breeze-ASR-26 & 258 & 24.27 &
32.29 & 27 & 22 & 0 & Optional dialect-aware comparator \\
\texttt{\seqsplit{whisper\_large\_v2\_test\_split}} & Whisper large-v2 & 258 &
24.72 & 32.23 & 33 & 28 & 1 & Comparable ASR baseline \\
\texttt{\seqsplit{whisper\_small\_test\_split}} & Whisper small & 258 & 34.86 &
...
\subsection{Table 2. Candidate And Exploratory
...
Evidence sources:
\codepath{70\_experiments/runs/asr\_candidate\_current\_recheck\_2026\_05\_26/candidate\_current\_recheck\_summary.tsv}
and
...
\subsection{Table 3. Human-Reviewed Predictor
...
\subsection{Table 4. Human-Reviewed Recovery Policy
...
and
\codepath{70\_experiments/runs/janus\_300\_high\_stakes\_recovery\_human\_reviewed\_2026\_05\_26/summary.json}.
Row-clustered uncertainty source:
...
\section{Figure Package}\label{figure-package}}

The figure package is generated from aggregate-only inputs by
\codepath{80\_semantic\_risk\_asr/paper/generate\_paper\_figures.R}. The Python
entrypoint
\codepath{80\_semantic\_risk\_asr/paper/generate\_paper\_figures.py} remains only
as a compatibility wrapper that calls the R script. The generated SVGs and PDF
exports and their privacy boundary are listed in
\codepath{80\_semantic\_risk\_asr/paper/figures/}. The aggregate artifact
manifest is generated by
\codepath{80\_semantic\_risk\_asr/paper/build\_artifact\_manifest.py} and
written to
...
\end{minipage} \\
\midrule\noalign{}
F1. CDS-ASR pipeline & \texttt{\seqsplit{figures/f1\_cds\_asr\_pipeline.svg}} &
...
F2. Evidence boundary & \texttt{\seqsplit{figures/f2\_evidence\_boundary.svg}} &
...
status only \\
F3. Predictor AUC & \texttt{\seqsplit{figures/f3\_predictor\_auc.svg}} & CEIS has
...
\texttt{\seqsplit{human\_audit\_predictor\_comparison.tsv}} & Aggregate predictor
metrics \\
F4. Recovery outcomes & \texttt{\seqsplit{figures/f4\_recovery\_outcomes.svg}} &
...
at the same 0.3889 budget. & \texttt{\seqsplit{policy\_comparison.tsv}} & Aggregate
policy counts \\
F5. Model lane state & \texttt{\seqsplit{figures/f5\_model\_lane\_state.svg}} &
...
state \\
F6. N-ladder & \texttt{\seqsplit{figures/f6\_n\_ladder.svg}} & The evidence chain
...
\texttt{\seqsplit{figures/f7\_budget\_risk\_frontier.svg}} & Fixed-budget policy
replay shows the trigger-budget tradeoff needed to eliminate severe
missed outcomes under scoped labels. &
\texttt{\seqsplit{fixed\_budget\_recovery\_frontier.tsv}} & Aggregate policy
counts \\
F8. Low-WER danger signals &
\texttt{\seqsplit{figures/f8\_low\_wer\_danger.svg}} & Low-WER rows can still
carry label-flip, unsafe-downrouting, and conservative-trigger signals,
directly supporting the title claim that low WER can remain operationally
dangerous. & \texttt{\seqsplit{low\_wer\_danger\_summary.tsv}} & Aggregate proxy
counts \\
F9. Risk-atom instability heatmap &
\texttt{\seqsplit{figures/f9\_risk\_atom\_instability\_heatmap.svg}} &
Instability concentrates differently by ASR model and decision-critical
atom, showing the mechanism behind CEIS/SRES risk signals. &
\texttt{\seqsplit{risk\_atom\_instability.tsv}} & Aggregate proxy counts \\
F10. Human-reviewed risk-atom outcomes &
\texttt{\seqsplit{figures/f10\_human\_reviewed\_atom\_outcomes.svg}} & Human
review connects risk-atom coverage to criticality and decision-change
evidence across reviewed rows. &
\texttt{\seqsplit{human\_audit\_risk\_atom\_review.tsv}} & Aggregate
human-reviewed counts \\
F11. Risk-atom entropy heatmap &
\texttt{\seqsplit{figures/f11\_risk\_atom\_entropy\_heatmap.svg}} & Atom-class
entropy summarizes whether instability is concentrated in a few atom classes
or distributed across several atom classes. &
\texttt{\seqsplit{risk\_atom\_instability.tsv}} & Aggregate entropy from atom
counts \\
\bottomrule\noalign{}
\end{tabularx}

\paperfigure{f1_cds_asr_pipeline.pdf}
{CDS-ASR converts audio into ASR hypotheses and runtime signals, extracts risk atoms, generates plausible variants, scores SRES/CEIS, and applies constrained recovery or conservative action.}
{method section}

\paperfigure{f2_evidence_boundary.pdf}
{The manuscript separates 258-row split/model-comparison evidence, selected-300 provenance evidence, and selected-300 human-reviewed predictor/recovery evidence.}
{publishable evidence summary}

\paperfigure{f3_predictor_auc.pdf}
{CEIS has the highest human-reviewed decision-change AUC in the scoped selected-300 audit, while SRES remains strongest on best-threshold F1; Table 3 reports row-clustered uncertainty.}
{human_audit_predictor_comparison.tsv}

\paperfigure{f4_recovery_outcomes.pdf}
{SRES-triggered recovery and CEIS-triggered conservative action both eliminate high-risk missed and critical miss counts in aggregate replay at the same 0.3889 budget.}
{policy_comparison.tsv}

\paperfigure{f5_model_lane_state.pdf}
{Main comparable split evidence, locale-gated candidates, and runtime-blocked probes remain separate until promotion gates are satisfied.}
{main/candidate aggregate summaries}

\paperfigure{f6_n_ladder.pdf}
{The evidence chain separates 258 rows, selected-300 provenance, 30 reviewed rows, and 90 clustered model assessments.}
{method evidence units}
...
{Low-WER rows can still carry label-flip, unsafe-downrouting, and conservative-trigger signals, directly supporting the title claim that low WER can remain operationally dangerous.}
{low_wer_danger_summary.tsv}

\paperfigure{f9_risk_atom_instability_heatmap.pdf}
{Instability concentrates differently by ASR model and decision-critical atom, showing the mechanism behind CEIS/SRES risk signals.}
...
{Atom-class entropy summarizes whether instability is concentrated in a few atom classes or distributed across several atom classes.}
{risk_atom_instability.tsv}

Submission-package PDF exports are generated with the same basename
under \codepath{80\_semantic\_risk\_asr/paper/figures/}.
...
selected row IDs, hypotheses, or reviewer notes. The R-generated F11 is
therefore the publishable entropy layer for the current evidence boundary,
while a future local-only confusion lexicon can support a supplemental
word-level entropy heatmap after privacy review.
...
\section{Claim Registry}\label{claim-registry}}
