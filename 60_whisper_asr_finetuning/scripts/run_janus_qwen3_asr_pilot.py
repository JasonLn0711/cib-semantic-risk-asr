#!/usr/bin/env python3
"""Run a JANUS pilot inference pass for Qwen3-ASR models."""
...
def dtype_for(torch_module: Any, name: str) -> Any:
    return {
...
def main() -> int:
    ...
    device_map = args.device_map or ("cuda:0" if args.runtime == "cuda" else "cpu")
    model_dtype = dtype_for(torch, args.torch_dtype)
    ...
    for index, row in enumerate(selected_rows, start=1):
        ...
        prediction = str(getattr(results[0], "text", "") if results else "").strip()
        qwen_language = str(getattr(results[0], "language", "") if results else "")
    ...
    write_metrics(
        metrics_path,
        {
            ...
            "notes": (
                ...
                f"disable_cudnn={args.disable_cudnn};torch_dtype={args.torch_dtype};"
